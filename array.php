...
<section>
    ...
    <?php
   //Нужно попробовать создать длинную строку и положить ее в переменную. Далее из данной строки брать подстроки и записывать их в переменные (в массивы), массивы класть в ассоциативный массив, получая в итоге двухмерный массив нужной длины - у которого элементы будут строки. Данная задача это тоже матрица, состоящая из строк и столбцов, только со строковыми значениями. Желательно ее решить без использования строковых функций
    $string_4 = "a good green apple used for good applepie";
    ...
    //Здесь чтобы массивы начинались с нулевого индекса, надо каждый раз запускать цикл, для формирования каждого массива и затем одномерные массивы через array_push сложить в двухмерный массив.
    for ($p = 0; $p <= strlen ($string_4); $p++) {
        if($p >= 0 and $p <=3) {
            $new_arr4_1[$p] = $string_4[$p];

        }

        if($p >= 4 and $p <=9) {
            $new_arr4_2[$p] = $string_4[$p];

        }

        if($p >= 10 and $p <=16) {
            $new_arr4_3[$p] = $string_4[$p];

        }

        if($p >= 16 and $p <=34) {
            $new_arr4_4[$p] = $string_4[$p];

        }

        if($p >= 33 and $p <=37) {
            $new_arr4_5[$p] = $string_4[$p];
    ...
    array_push($main_arr4, $new_arr4_1);
    array_push($main_arr4, $new_arr4_2);
    array_push($main_arr4, $new_arr4_3);
    array_push($main_arr4, $new_arr4_4);
    array_push($main_arr4, $new_arr4_5);

    echo "Создали двухмерный массив со строковыми значениями";
    echo "<pre>";
    print_r($main_arr4);
    echo "</pre>";

//    for ($a = 0; $a <= count($main_arr4); $a++) {
//        for ($s = 0; $s <= ); $s++) {
//            if($main_arr4[$a][0] == 'a') {
//                echo "Массив начинается с элемента A";
//            }
//        }
//    }
//Сделано
